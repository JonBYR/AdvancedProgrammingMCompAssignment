...
cv::Mat ColourConvert::process() throw(InvalidOperationException) {
    if (filterType == "greyscale") {
        ...
        for (int i = 0; i < rows; i++) {
            threads.push_back(thread([=]() { //[=] captures everything used by the class, copying by values, including variables i and adjustedImage which are required in the lambda
                for (int j = 0; j < adjustedImage.cols; j++) {
                    ...
                    double red = bgrVal[2]; //lambda used as thread constructor requires function that each thread must process, which can include anonymous ones
